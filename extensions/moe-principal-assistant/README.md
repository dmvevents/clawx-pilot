...
**Pilot-ready.** The plugin registers principal-admin tools and exports a system
...
- **Tenant-native form API submission.** The current pilot path fills Forms in
  the browser session. The future Power Automate/Graph path can replace that
  transport without changing the agent-facing tool sequence.
