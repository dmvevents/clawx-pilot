...
7. **Iterate narrowly on the VM when it shortens the loop.** Use the VM development workspace only after binding it to the release checkout and artifact under test. Keep fixes small, return reviewed diffs to the main checkout, and do not promote ad hoc diagnostic scripts to operational defaults until they land in the repository and pass review. For long Windows SSH work, keep the connection alive through completion or use an explicitly owned durable task with a result receipt. A detached `Start-Process` acknowledgment did not establish execution in this lab. Capture the native exit separately from the SSH exit; after interruption, inspect logs, exit receipt and owned processes before resuming. Stop only verified owned PIDs/process groups; never broad-kill SSH/IAP/gcloud or app names. On the tested Server image, use legacy SCP `-O` with hash readback; use a copied `.ps1` for long commands instead of the stalled stdin/base64 fallback. Reuse hash-verified tools/dependencies. See the native-development procedure in `docs/testing/WINDOWS_REPEATABLE_LAB.md`.
