...
5. **Prefer a fresh standard user for vanilla-machine claims.** A fresh or reused Server 2022 profile can prove installer and runtime mechanics, but neither establishes Windows 10/11 client acceptance. Standard-user setup must preserve ACL boundaries and avoid requiring elevation for normal first run.
