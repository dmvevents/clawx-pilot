...
acceptance:
  ...
  - The macro is invoked at the existing pre-extraction replacement point in customCheckAppRunning, after app shutdown handling, and performs filesystem replacement/path checks only (no process kills, registry writes or user-profile writes).
  ...
  - Native behavioral acceptance (baseline stale-file reproduction, replacement with sibling/profile preservation, locked-path abort) is executed by the independent test author's harness that includes this exact macro, and remains root-owned; this spec does not claim installed-app proof.
docs:
  required: false
---
