...
acceptance:
  ...
  - customInstall performs NO post-install deletion of the rollback tree. The former delayed `cmd.exe /c ... rd /s /q "$ClawXStaleInstallDir"` is removed because cmd expands a literal %VAR% inside the quoted path (native read-only control 2026-09-08 22:14:52 UTC, artifacts/ga-fable-20260908/windows-lab/native-moe27-upgrade/quoted-percent-cleanup-path-control-v1.json), so a rollback path containing a literal percent token could resolve to and recursively delete a DIFFERENT similarly named sibling directory. Instead the exact $ClawXStaleInstallDir rollback tree from this invocation is retained after a successful install with a DetailPrint notice; the disk-space cost is an accepted tradeoff and reclaiming it is an explicit later operator/support action (procedure documented by root, outside this spec). No shell-escaping layer, cleanup service, user confirmation flow, or wildcard deletion of arbitrary ._stale_* or sibling directories is introduced.
