...
acceptance:
  ...
  - The unsafe/root probe (unmapped drive letter only) runs first and gates the rest of the suite; because native NSIS startup validation reverts an invalid /D= bare root to the compiled placeholder before .onInit (proven 2026-09-08, assembly b8423f22), the fixture binds $INSTDIR to the runner's exact intended root only under a strict test-only guard (byte-match to CLAWX_FIXTURE_TARGET, bare "<letter>:\" shape, OS-absent root; violation exits 6), the normal target-mismatch guard still verifies the bound value, and the scenario must prove the ACTUAL macro was reached and itself rejected the root (exit 2) — a target-mismatch exit never counts as a macro pass; the locked-old-file scenario proves the Windows rename refusal with an owned negative-control rename (child-file handle, directory-handle fallback, restore on unexpected success) before asserting the macro fails closed.
