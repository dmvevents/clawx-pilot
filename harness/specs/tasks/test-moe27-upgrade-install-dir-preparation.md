...
acceptance:
  ...
  - The unsafe/root probe (unmapped drive letter only) runs first and gates the rest of the suite; because native NSIS startup validation reverts an invalid /D= destination to the compiled placeholder before .onInit (proven 2026-09-08, assembly b8423f22, and by the 22:43 UTC startup-only control for BOTH a bare drive root and an existing plain file), the fixture binds $INSTDIR to the runner's exact intended target only under strict test-only guards (byte-match to CLAWX_FIXTURE_TARGET; for the root probe a bare "<letter>:\" shape whose letter is unmapped per GetLogicalDrives with query failure refused — never an enumeration/IfFileExists absence claim; for the plain-file probe exactly "<result-file directory>\install-dir" pinned in the runner-created scenario directory and currently an existing plain non-reparse file; any violation exits 6), the normal target-mismatch guard still verifies the bound value, and both scenarios must prove the ACTUAL macro was reached and itself rejected the destination (exit 2) — a target-mismatch or binding-guard exit never counts as a macro pass; the locked-old-file scenario proves the Windows rename refusal with an owned negative-control rename (child-file handle, directory-handle fallback, restore on unexpected success) before asserting the macro fails closed.
