...
校長が Chrome やブラウザを開くよう求めた場合、アシスタントは明示的な `browser.open_chrome` ツールを呼び出します。これは既存のメインプロセス修復サービス（`POST /api/browser/repair-chrome-cdp`）を通じて校長のシステム Chrome を開きます。Ministry のジャーニーで内蔵のマネージドブラウザを使うことはありません。Windows では、CDP エンドポイントが HTTP 応答しても、有界のループバック所有権チェックでリスナーが現在の Windows セッションかつ ClawX 専用自動化プロファイルで動作していると確認できて初めて「準備完了」と報告します。他セッション/他プロファイルの所有が確認された場合や所有者を特定できない場合は、型付きの拒否（`foreign_endpoint_owner` / `endpoint_owner_unverified`）と Windows 向けの案内を返します。ClawX が他ユーザーの Chrome を閉じたり接続したりすることはなく、勝手に別ポートへ切り替えることもありません。
