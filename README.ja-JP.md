...
会話の処理が進まなくなった場合は、エラーを表示して入力欄を再び利用可能にします。保存済みのオンラインアカウントへの復帰には、プロバイダーの確認とセッション更新の成功が必要です。進行状況とキャンセルは、対象の会話ターンに結び付けて管理します。Gateway が接続済みでも利用可能なモデル経路がない場合、入力欄は設定案内を表示したまま無効になり、タイムアウトする送信を開始しません。自動の「このデバイス」フォールバックには、設定済み Ollama モデルの readiness 通過が必要です。ユーザーが明示的に選んだ「このデバイス」設定は保持されます。

Windows バイナリの準備では、固定バージョンの LGPL FFmpeg とライセンス・出所情報を取得し、Windows 上で音声変換を検証します。パッケージ作成の前後に必須ツールを確認し、配布する `resources/bin` ディレクトリのハッシュをリリースマニフェストに記録します。

Ministry 校長アシスタントは、内容を読む前に `document.find` でローカル文書名を解決します。検出では有界なメタデータと `safeUnique` だけを返し、選択されたファイルだけを正確な `document.read_*` ツールで読みます。候補が曖昧、または検索が不完全な場合は校長に選択を求めます。

メール機能に関する質問（「Microsoft Graph はインストール/サインイン済みか、メールはどの経路を使うか」）には、読み取り専用の `outlook.readiness` ツールが `POST /api/outlook/readiness` を通じて答えます。型付きの Graph 状態（サインイン済み・未サインイン・未設定・不明）と、読み取り/作成それぞれのトランスポート選択を、実際のメール呼び出しと同じ選択コードから報告します。ウィンドウは開かず、メールボックス API も呼びません。Outlook ウィンドウ自体のサインイン状態は `outlook.open` が観測するまで正直に「不明」のままです。状態の読み取りに失敗した場合は「不明」と報告し、決して「未設定」とは報告しません。設定ファイルが存在しないことを Graph サポート不在の証拠として扱うこともありません。

校長が Chrome やブラウザを開くよう求めた場合、アシスタントは明示的な `browser.open_chrome` ツールを呼び出します。これは既存のメインプロセス修復サービス（`POST /api/browser/repair-chrome-cdp`）を通じて校長のシステム Chrome を開きます。Ministry のジャーニーで内蔵のマネージドブラウザを使うことはありません。Windows では、CDP エンドポイントが HTTP 応答しても、有界のループバック所有権チェックでリスナーが ClawX が駆動してよい Chrome — ClawX 自身が起動したプロセス（PID による正の所有確認）、ClawX 専用自動化プロファイル、または校長本人がデバッグポート付きで意図的に起動したユーザープロファイルの Chrome — であると確認できて初めて「準備完了」と報告します。Outlook / Forms ドライバも CDP 接続の前に同じ所有権チェックを実行し、所有権・起動・準備確認はエンドポイントから導かれる単一のポート識別を共有します。別の Windows ユーザーセッションの所有と同一セッション内の競合は区別された正確な案内（`foreign_endpoint_owner`）を返し、所有者を特定できない場合は偽の準備完了ではなく `endpoint_owner_unverified` として拒否します。校長がすでにサインインしているセッションへのサインインを求めることはなく、身元不明を理由にプロセスを終了することもなく、ClawX が他ユーザーの Chrome を閉じたり接続したり、勝手に別ポートへ切り替えることもありません。
...
- **Node.js**: 24.x、24.15.0 以降（選択した OpenClaw ランタイムに合わせる）
