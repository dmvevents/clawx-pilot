...
2. **Outlook `send_email` requires hard confirmation:** normal reviewed sends use `outlook_send_email({confirm:true})` only against the single visible reviewed draft. Optional recipient/subject/body args are safety assertions for advanced flows, not the normal demo path.
