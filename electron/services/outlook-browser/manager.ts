  ...
  async open(): Promise<OutlookOpenResult> {
    ...
    if (!status.running) {
      return {
        ...
        message:
          'Ministry of Education could not attach to Chrome for Outlook. The assistant should run browser_diagnose and browser_repair_chrome_cdp, then retry outlook_open. If repair reports profile_locked_close_chrome, close all Chrome windows and retry from Ministry of Education. Do not ask the principal to configure Chrome manually.',
      };
    }
