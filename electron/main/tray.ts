...
export function createTray(mainWindow: BrowserWindow): Tray {
  ...
  // Set tooltip
  tray.setToolTip('Ministry of Education - Assistant');
  ...
  const contextMenu = Menu.buildFromTemplate([
    {
      label: 'Show Ministry of Education',
      click: showWindow,
    },
    {
      type: 'separator',
    ...
    {
      label: 'Quit Ministry of Education',
      click: () => {
        app.quit();
      },
    },
...
export function updateTrayStatus(status: string): void {
  if (tray) {
    tray.setToolTip(`Ministry of Education - ${status}`);
  }
}
