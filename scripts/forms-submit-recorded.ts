/**
 * Screen-recorded, verified submission harness for the Suspensions CLONE form.
 *
 ...
 *      production SuspensionsActions driver (expect ~29/32).
 ...
 *   9. Saves all evidence to skills/laptop/evidence/2026-09-03-forms-submit-recorded/.
 *
 * Run:
 *   pnpm exec tsx scripts/forms-submit-recorded.ts
 *
 * Reuses the production driver (electron/services/forms-browser-v2) — the same
 * code path the agent uses. Payload mirrors scripts/forms-fill-suspensions.ts.
 */
...
const ALLOWED_URL_FILE = join(REPO_ROOT, 'extensions/moe-principal-assistant/forms/suspensions-test-fac-url.txt');
const EVIDENCE_DIR = join(REPO_ROOT, 'skills/laptop/evidence/2026-09-03-forms-submit-recorded');
...
class Recorder {
  ...
  async start(page: Page): Promise<void> {
    ...
    try {
      await page.context().tracing.start({ screenshots: true, snapshots: true, title: 'forms-submit' });
...
// ---------------------------------------------------------------------------
// Payload — mirrors scripts/forms-fill-suspensions.ts (synthetic test data).
// ---------------------------------------------------------------------------

const SAMPLE_PAYLOAD: SuspensionsPayload = {
  respondent_name: 'Test Principal (recorded harness)',
...
async function main(): Promise<void> {
  ...
  try {
    ...
    mark('fill: driving 32-field payload through SuspensionsActions');
    ...
    if (fill.filledCount < 25) {
      fatal(5, `fill landed only ${fill.filledCount} fields (<25) — aborting before any submit`);
    }
