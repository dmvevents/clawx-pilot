...
## Native installer upgrade regression (CLWX-135)

Use the reviewed source worktree's `tests/windows/nsis-upgrade/` harness before another installer build after a replacement/cleanup change. It compiles the actual preparation macro and post-uninstall hooks; it must never invoke the installer's process shutdown or system-integration macros. A fixture pass is filesystem proof, separate from the subsequent installed-app upgrade, startup and ordinary-chat acceptance.

1. Record the source and harness revisions and source-file hashes. Reuse the existing pinned NSIS compiler and its explicit `NSISDIR`; run `tests/windows/nsis-upgrade/compile-fixture.sh scripts/installer.nsh`. Preserve the compiler receipt and executable SHA256. Do not accept a stand-in macro compile as behavioral evidence.
2. Hash-check the executable and PowerShell runner after transfer. Parse the runner with the guest's Windows PowerShell 5.1 before execution. Use an identified **standard-user** token and a new owned fixture directory; the harness currently requires a path without whitespace. Keep all synthetic trees outside installed app, actual AppData, `.openclaw` and Chrome directories.
3. Run the reviewed eleven-case suite sequentially. Cover missing/empty destinations, unrelated directories/files, junction preservation, stale runtime replacement, collision siblings, profile sentinels, real locked-path refusal, repeated/inner-only post-uninstall hooks, and denied enumeration. A control that cannot actually deny enumeration or prevent rename is invalid and must fail; it cannot be counted as a pass or skipped required row. The lab operator's ACL control did not enforce denial; the standard QA account did.
4. Bound each fixture wait and the enclosing process/task budget; capture native exits separately from the observer task's exit. Keep the complete suite summary and per-case results, source/fixture identities, account class, timestamps, restored ACL/handle evidence and failures. Preserve failed attempts and use a new fixture directory only after diagnosing the failure. No blanket process kills or cleanup of neighboring folders.
5. Continue to a uniquely identified installer only after independent source/harness review and all required native cases pass. Test a normal upgrade over the preserved old installation, the actual desktop shortcut and ordinary chat. Server 2022 fixture results do not satisfy fresh Windows 10/11 or unaided stakeholder acceptance.

### Retained rollback folders

The replacement repair retains the exact previous installation at the sibling path reported by the installer (`<install-directory>._stale_<n>`). This costs disk space and preserves recovery material while the new version is validated. Automatic shell deletion was removed because even a quoted path containing a literal `%VAR%` can expand to a different folder in `cmd.exe`.

Operator/support cleanup is a separate action after the new installed version passes acceptance and the rollback is no longer needed. Record the exact retained path, verify it is the recorded sibling of the active installation, inspect it and its ancestors/contents for reparse links, confirm no process runs from it, and retain required failure evidence or a recovery archive. Then remove only that individually verified tree through a literal-path filesystem operation. Do not interpolate it into a shell command, remove wildcard `._stale_*` matches, or delete a folder merely because its name looks stale. Abort cleanup when ownership or path identity is uncertain. Current failed installations and diagnostic fixtures remain preserved until their investigation is complete.

See [CLWX-135's evidence and failed controls](../bugs/CLWX-135-openclaw-package-lifecycle.md) for the tested revisions and current remaining gates.
