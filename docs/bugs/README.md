...
| CLWX-61 | [Attachment metadata fabricates an empty filename](CLWX-61-attachment-metadata-empty-filename.md) | Reproduced at source `0f708082`: a whitespace-only chip label yields an entry with an empty filename that can reach the gated retrieval action; the confirmation gate itself is unchanged; fix not attempted |
