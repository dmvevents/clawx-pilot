...
### Moe.28 installer UI observation recovered after RDP disconnect — September 8, 23:27 UTC

The initial QA Session 2 installer inspection failed with `The handle is invalid`; native session readback showed the existing QA session disconnected and `LogonUI` present. This observer result is FAIL, not a failed product install. The identified moe.28 installer PID4936 remained running, and all695 protected profile-backup files matched. No installer command had been staged or clicked.

Root read the RDP certificate SHA256 through authenticated IAP SSH, verified the existing loopback tunnel's RDP protocol, and reconnected FreeRDP to the same standard user with the password on stdin and certificate pinning. The second session readback was Active in the original Session2. At23:27:59UTC the unchanged observer successfully captured the actual moe.28 License Agreement, PID4936 and enabled I Agree control. No app/profile reset, browser restart, duplicate installer or source modification was used. The disconnection's original cause remains UNKNOWN; it must not be attributed to product code without evidence.

Private receipts: `artifacts/windows-vm/20260908-moe28/{installer-ui-handle-diagnostic,rdp-current-certificate,rdp-client-receipt,rdp-session-restored-readback-v2,installer-reconnected-ui-readback,operator-preinstall-observation}.json`. Next-agent rule: require an active identified interactive session before screenshot/UI operations; recover that session before interpreting an invalid desktop handle as an app defect. Native installation, shortcut startup and chat remain separate acceptance results on CLWX-135.
