...
## Independent review of 41359e1 — September 8

Verdict APPROVE, source-only: reviewer reran the 240-test focused set, typecheck/lint and harness validation. All six assigned ownership/attach/compose findings passed their checks. Installed multi-session Windows and live tenant behavior remain NOT_RUN; the original stock-timeout cause remains UNKNOWN. Review: `artifacts/ga-fable-20260908/browser-ownership-review/result.json`; reproducible probes and detailed report are in `/private/tmp/clawx-browser-ownership-review-20260908/artifacts/review-20260908/`, source `41359e121c808ecb42ecc9e307e12892fb895fc8`.

New non-blocking reviewer finding L1: `commandUsesUserOwnedProfileDir` in `electron/services/chrome-cdp.ts` uses a home-directory prefix without a path-separator boundary. A controlled row with home `C:\Users\Teacher`, command profile under `C:\Users\Teacher2`, and no reported session ID yields `cdp_ready`; expected unverified/refused ownership. The reviewer reproduced this in its adversarial probe P1. Actual shipped PowerShell returns commandLine and sessionId from the same process record, so the incomplete-record combination was judged effectively unreachable; readable unequal sessions are already refused. This is a known source boundary weakness, not installed exploitation evidence. Suggested bounded correction: require home itself or its separator-delimited descendants and pin the prefix-collision negative case. Root retains this follow-up under CLWX-130; no duplicate card or additional review of unchanged code is required. Any correction still needs focused evidence and independent review before integration.

## Assisted Windows rerun — September 8, 14:55–15:06 UTC

After the owner released the RDP hold, root verified the original guest identity through strict SSH host-key trust and pinned-certificate RDP. Installed moe.25 app PID 388 was in standard-user Session 2; user-Chrome CDP 18792 was still owned by operator Session 1, PID 4860. Root gracefully closed only that verified operator ClawX CDP profile, preserving the app and the separate managed browser. The installed app's `/api/outlook/open` then succeeded in 5,421 ms. Readback verified new Chrome PID 2748 in Session 2 and its expected user-owned profile, not managed Chromium.

QA sign-in, visible exact account/inbox verification, installed Host API read (three messages, 8.7s) and a fresh Online chat with confirmed `outlook.read_inbox` followed successfully. The chat took 95s and passed a 30-second stable-terminal observation. [Full scoped evidence](../evidence/WINDOWS_REPEATABLE_LAB_2026-09-08.md) links private receipts and exact moe.25 baseline.

This proves an assisted recovery from the reproduced foreign-session endpoint condition. It does not establish the original 11:44 stock-browser timeout cause, install reviewed guard `41359e1`, or satisfy the final-candidate wrong-session/normal-user regression. Those remain the next installed acceptance checks after the new build.
