...
- Reported/last verified: 2026-09-09, discovered by the CLWX-61 authoring lane while writing the DOM-fixture unit pin; independently reproduced by the suite's own assertion at author time. The pin (`cd88a205`) and the repair (`2b5646a5`) are both independently approved; the repair is integrated into candidate `0.4.3-moe.30` as `7dac940d`.
...
- Status: **reproduced at source, repaired, repair independently approved, integrated into candidate `0.4.3-moe.30`.** Remaining gates: the built artifact must carry it, then the installed and live-account legs that CLWX-61 already owns.
