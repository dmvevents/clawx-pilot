...
## Guarded-suite helper-process evidence (September 8, 2026)

The bounded read-only Claude receipt (`artifacts/ga-fable-20260908/keychain-recurrence-receipt/result.json`; detailed private report in `/private/tmp/clawx-e2e-macos-keychain-20260908/artifacts/keychain-recurrence-analysis.md`) completed after the initial investigation timed out. Model/provider metadata confirms Fable 5 on Bedrock for the receipt.

Unified logs distinguish the guarded suite from its predecessor: browser-process `SecItemAdd` errors disappeared with the fixture flags, while plain Electron Helper utility processes 67473/68005/69397 still called `SecItemCopyMatching` then `SecItemAdd`, returning `-25307` about 180 ms after helper startup. Helpers started 9–16 seconds after the application, matching the 17:27–17:28 Dubai screenshots. The browser-only guard was insufficient. The precise requesting component inside the helper is UNKNOWN; network-service key initialization is the leading hypothesis, not a proved component attribution.

The prior synthetic launches lasted about five seconds and missed late helper startup. Instantaneous zero-window observations also missed ephemeral SecurityAgent presenters. Neither result establishes absence of a prompt across a full suite.

Next proposed correction: evaluate an isolated test runtime containing upstream Electron PR 50419, without changing shared dependencies. A future controlled long-lived test must monitor keychain-add failures across the entire owned process tree for more than 30 seconds before the held suite can establish a pass. No corrected runtime or controlled rerun was executed by this investigation. Owner Mac desktop launches remain held; do not resume them from this source-only recommendation.
