...
## Fix rounds arising from the reviews — all closed

Every review that returned findings produced a repair, and every repair was verified by the lane that raised the finding.

| Card | Repair | Outcome |
|---|---|---|
| CLWX-130 | `75a40554` — real path containment (`isPathWithinDir`) replaces the string prefix, so another user's directory can no longer read as ours; the attach gate probes `/json/version` on `no_listener` and refuses if anything answers; the inline PowerShell runs under `-ErrorAction Stop` with only a not-found condition reaching `no_listener` | **APPROVE.** Reviewer reproduced the pre-repair failure (18 failed / 50 passed) without dirtying a worktree, examined the OR-versus-AND decision and accepted it, and named the one scenario that defeats OR — a same-user stale browser in a disconnected session with partly unreadable metadata — which is recorded as a known limitation rather than "fixed" by a change that would cause a real false refusal. Lead separately confirmed the fetch helper returns rather than throws on a non-success status, so an answering endpoint is refused |
| CLWX-61 pin | `cd88a205` — the `it.fails` row became an exact pin on the defective output | **APPROVE.** The reviewer's own extra mutant had shown a substituted default name left the whole suite green; after the change both the repair and the substitution turn exactly that row red |
| CLWX-61 defect | `2b5646a5` — `if (!filename) continue;` guards the push, regex untouched; the pin inverted with a valid chip beside the malformed one so it also fails if the guard over-drops; a placeholder row added | **APPROVE.** Three mutants each fail exactly the two intended rows, including the reviewer's label-derived default. The reviewer also **corrected the lead's severity claim**: the empty name passes the retrieval existence check, but the locator built next is an empty substring selector that matches nothing, so the real cost was a thirty-second hang and a misleading refusal, not a wrong retrieval. Corrected in the report with the original overstatement left visible |
| CLWX-102 guard | `7615c43b` — fail-closed refusal at the three entry points that can start a real backend process | **APPROVE.** Reviewer proved exhaustiveness (exactly three fork sites, every start and restart caller funnelling through one method, the remaining execPath spawns already Node-mode and excluded from test mode) rather than checking only the sites it was handed |
| CLWX-102 orphan scan | `8923f132` — the refusal moved to the top of `start()`, ahead of the orphan scan, the Python warmup and device-identity I/O | **APPROVE.** This was raised as a non-blocking follow-up and promoted to a fix round because it defeated the guard's own purpose: the scan `lsof`s the fixed port and SIGTERM/SIGKILLs anything unowned, so a test run would have killed the operator's live backend *before* the guard refused. The regressions now assert nine helpers are uncalled rather than merely that no fork happened — asserting absence-of-fork is what let the gap pass the previous review. Pre-change reproduction: three rows fail on the scan having been called once. The per-run-port alternative was refused with eleven hard-coded sites enumerated, and the reviewer independently checked them and accepted |

### What the fix rounds say about the review process

Four of the six review lanes returned findings rather than sign-off, and three of those findings were invisible to the checks that preceded them:

- A test pin that read as rigorous (`it.fails`) could not distinguish the defect it documented from a *different* fabrication on the same gated path. Only a mutant the author had not tried exposed it.
- A guard that genuinely covered all three process-fork sites still allowed a test run to terminate the operator's backend, because the hazard sat one layer earlier than "fork". The review that approved it had checked fork sites, as asked.
- A severity claim written by the lead overstated impact until a reviewer traced two steps further into the same function.

The common shape: each was caught by asserting on what must *not* happen, or by tracing one layer past the obvious boundary. That is worth more than the sign-offs.
