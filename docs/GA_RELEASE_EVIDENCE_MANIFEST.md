...
| Clean Windows install / VM smoke | YELLOW pending assisted install | VM is reachable over WinRM/IAP, runner scripts were refreshed with the Outlook state matrix, and the VM downloaded the new GitHub asset with size `390061316` and SHA256 `2e189dd004995d6ce18e9e240f8228ba5039c2e384fd479a597137458a9046cf`. The hidden install path did not produce an app exe, so installed-app Outlook/Forms smoke could not run from WinRM. | VM `clawx-win-rc-20260609`; download path `C:\Users\codexvmtest\Downloads\clawx-e2e-runner\Ministry.of.Education-0.4.3-moe.10-win-x64.outlook-reply-fix.exe`; runner path `C:\Users\codexvmtest\Downloads\clawx-e2e-runner` | Codex | Test credentials handled through environment variables only; no passwords printed; no email sent; no attachment downloaded; no Forms submitted | Requires normal assisted desktop/RDP install, then test-account Microsoft sign-in and `pilot-managed-cdp-visual-smoke.ps1 -OutlookStateMatrix` with Forms no-submit smoke |
...
- The current refreshed GitHub installer asset was verified on the VM: `C:\Users\codexvmtest\Downloads\clawx-e2e-runner\Ministry.of.Education-0.4.3-moe.10-win-x64.outlook-reply-fix.exe` had size `390061316` and SHA256 `2e189dd004995d6ce18e9e240f8228ba5039c2e384fd479a597137458a9046cf`.
- Refreshed installed-app smoke is pending for this exact asset. VM state before assisted install: `codexvmtest` had no app installed, while an older app existed under the `clawxtest` profile.
