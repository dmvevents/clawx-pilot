...
| Status note | Rebuilt locally on 2026-06-23 at `ea72db2` after Outlook reply/body and archive-adjacent Reply regressions, then uploaded to the GitHub prerelease. Local package inspection, focused 92-test Outlook/plugin/probe release slice, full unit suite, typecheck, lint, and harness CI passed. Packaged `app.asar` inspection found the new reply recipient-field/body verification guard. Installed-app Outlook/Forms smoke must still run against this exact asset before GA. |

## Gate Table

| Gate | Status | Command / evidence | Artifact path | Verifier | Redaction notes | Blocker / accepted risk |
|---|---|---|---|---|---|---|
| Local unit regression matrix | GREEN | `pnpm test` passed: 147 files, 1091 passed, 5 skipped | terminal output | Codex | No secrets printed | MaxListeners warnings observed; no failures |
...
| Outlook reviewed-draft send | GREEN for unit/source | `tests/unit/outlook-actions-safety.test.ts`, `tests/unit/moe-principal-assistant-plugin.test.ts`; focused Outlook/plugin/probe suite passed 127 tests | repo tests | Codex + test-engineer sidecar | No email sent | Installed signed-in Outlook proof still required for GA |
...
| Local signed-in Outlook CDP manager smoke | GREEN for local source/no-install | Local Chrome at `outlook.cloud.microsoft` on CDP `18792`: `scripts/v2-smoke.ts` returned `open=opened`, `readInbox(5)=ok`; custom read-only probe returned `readInbox(50)=12`, June filter `10`, May filter `2`, Raj filter `2`, all with `scan.exhaustive=false` | terminal output | Codex | No email bodies, recipients, passwords, tokens, sends, downloads, or Forms submits | This proves the source browser-manager path only. It is not installed-app proof and is not in the listed GitHub installer until the next package/upload |
...
| Clean Windows install / VM smoke | YELLOW pending assisted install | VM is reachable over WinRM/IAP. Previous VM evidence belongs to the superseded `2e189dd...` GitHub asset. The refreshed `dbd252...` installer is uploaded to GitHub but has not yet been refetched/installed in the VM, so installed-app Outlook/Forms smoke remains pending for this exact build. | VM `clawx-win-rc-20260609`; runner path `C:\Users\codexvmtest\Downloads\clawx-e2e-runner` | Codex | Test credentials handled through environment variables only; no passwords printed; no email sent; no attachment downloaded; no Forms submitted | Refetch the `dbd252...` asset, perform normal assisted desktop/RDP install, then run test-account Microsoft sign-in and `pilot-managed-cdp-visual-smoke.ps1 -OutlookStateMatrix` with Forms no-submit smoke |
...
- Fresh install package smoke is suitable for prerelease publication; the GitHub asset has been refreshed to the `dbd252...` installer, and the VM still needs to refetch that exact file.
- Source-only local Outlook CDP evidence after the listed asset: with the user's local Chrome already open at `https://outlook.cloud.microsoft/mail/`, the v2 browser manager opened the signed-in mailbox and read the Inbox successfully. The widened date-search probe split the 12-row bounded recent Inbox window into 10 June rows and 2 May rows, which directly validates the no-year Outlook date parser fix for the "missing older June emails" regression. This evidence does not update the GitHub installer by itself.
