...
7. A fresh ordinary **Online chat** requested only the count and unread count of the three newest messages. It returned “3 messages, all unread,” with no error chip, and passed the driver's 30-second terminal stability check. Answer latency was **95,044 ms**, one sample. The renderer receipt listed no tool names, so root checked the actual Gateway session: `de46b8e9-8206-48ef-8fdf-60a48012c072.jsonl` records **`outlook.read_inbox` at 15:05:42.350 UTC**. Provider/model route labels were `custom-moecloud` / `moe-demo-pro`; those labels alone do not identify the cloud model behind the broker.
...
Claude Fable 5 on Bedrock owns source integration and a separate repair of the QA login helper's URL-only false-PASS condition. The root operator owns VM mutation, evidence and release decisions. Integrate reviewed source, identify/build the next installer, then rerun the affected installed journeys and end-user guide. No source or infrastructure result grants GA readiness.
