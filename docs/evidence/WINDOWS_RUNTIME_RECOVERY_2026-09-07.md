...
The [third native run](https://github.com/dmvevents/clawx-pilot/actions/runs/34121639939), source `fed34dcb55ba58c4996ce74c7d4a874698d60f75`, passed **201 test files, 2,007 tests and 11 platform skips**, plus the remaining preflight and frontend/Electron compilation. Packaging stopped at the actual UtilityProcess PDF probe: its repository fixture at the runner's `D:\a` checkout is outside the document parser's home/temp sandbox. No installer was produced. The correction stages the public fixture in an owned OS temp directory and preserves real PDF parsing, expected-text verification, missing-fixture failure and existing runtime access restrictions. README files were reviewed; this corrects a build probe and does not change the product interface.

Independent source review is clear. Six execution regressions pass in the actual public checkout with no staged OpenClaw build: real PDF parsing from temp, forbidden-path refusal, worker temp cleanup, verifier staging and missing-fixture failure. The actual local staged-bundle verifier also passes under isolated home/temp settings; neither probe leaves an owned temp directory behind. Scoped lint and typecheck pass. The next native package run remains required.


## Pre-native lifecycle and transport integration — September 7

The fourth native run passed 201 files / 2,011 tests and seven artifact rows before the all-plugin diagnostics sweep timed out. Its scoped-loader repair now passes both native Windows diagnostic inventories under the unchanged 120-second limit: 42,413ms for cold no-hostapi and 6,094ms for full-hostapi. Exact host/guest script hashes agree. This used a private copy of pinned installed OpenClaw with current plugin source, not a new installer.

CLWX-117/96 source now denies on-device `sessions_yield`, terminalizes stalled partial/tool-only work on owned progress, ignores empty heartbeats, and cancels the exact stalled or late-acknowledged old run. Recovery of an inactive same-session fallback requires a Main-owned, quiet, aborted-on-timeout saved-provider check with a strict 2xx response and a confirmed Gateway clear. Session, generation, channel, account, pin and notice ownership are checked around asynchronous work.

Repeated-outage tests caught and corrected a session-lifetime retry budget. Unavailable provider checks no longer spend mutation attempts; successful automatic recovery and newly acknowledged fallback pins reset the clear budget. Three failed clear acknowledgements remain bounded within one episode; explicit channel clear resets that budget. Tests include three unavailable probes followed by a healthy fourth and more than three fallback/recovery cycles in the same app run.

Final changed behavior passed 117 focused tests, six Electron interactions, typecheck, scoped lint, actual public-diff task validation/dry run, communication replay/comparison and independent review. The earlier combined local preflight passed 203 files / 2,047 tests with six skips; harness CI retained its explicit live-model selection gap. None of this closes the installed moe.20 failures or establishes moe.21 acceptance. The next complete native package is the source-freeze exit gate.
