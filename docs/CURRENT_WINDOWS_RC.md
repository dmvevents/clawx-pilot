...
The third native moe.21 [build attempt](https://github.com/dmvevents/clawx-pilot/actions/runs/34121639939), source `fed34dcb55ba58c4996ce74c7d4a874698d60f75`, passed all 201 Windows test files (2,007 tests, 11 platform skips), preflight and frontend/Electron compilation. It stopped before installer creation because the PDF probe used a checkout path outside the parser sandbox. The reviewed temp-staging correction passes six execution checks in the clean public checkout without widening runtime access. Earlier CRLF, subprocess and test-budget failures are cleared by this native run. See the [runtime report](evidence/WINDOWS_RUNTIME_RECOVERY_2026-09-07.md#native-windows-preflight--september-7).
