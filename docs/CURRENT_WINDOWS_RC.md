...
**September 8 first-response RCA checkpoint, 07:48Z:** reviewed repairs in diagnostic source `8ca085f4` reach the actual Online model after 75.061 seconds, versus 112 seconds in the original failure. The submitted prompt now survives timeout, but the first answer still fails: cancellation follows after only 24.933 seconds of model execution. The driver proves a fresh session and the transcript binds the cloud provider. All original installed files were restored with matching hashes at 07:47:52Z. Source inspection identifies a dropped lifecycle `start` event; the next repair will preserve strict run ownership and the existing timeout. This is diagnostic evidence, not acceptance of a new installer. **GA remains RED; CLWX-125 remains In Progress.** The [RCA and flow map](evidence/WINDOWS_FIRST_RESPONSE_RCA_2026-09-08.md) records history, code samples, tests and VM setup.
