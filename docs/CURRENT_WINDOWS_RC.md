...
**September 9, later: GA RED; the owner resumed execution and the current candidate is `0.4.3-moe.30`.** [COMPLETION_PLAN.md](COMPLETION_PLAN.md) owns the current outcome. moe.30 (`40cfe727`, branch `release/moe30-integration`) integrates three independently approved repairs that existed only in author worktrees — CLWX-61 attachment extraction, CLWX-130 browser identity and attach fail-opens, and the CLWX-102 test-seam guard — with `patch-id` equivalence to the approved originals and an independent integration APPROVE. Source gates pass: typecheck, lint, full unit suite 2,444 passed with zero failures, comms replay/compare unchanged, diff-aware spec validation. Its hosted build [34323058772](https://github.com/dmvevents/clawx-pilot/actions/runs/34323058772) succeeded on the exact candidate SHA, and the package is identified: installer SHA256 `9f8a2dc5…0114aff7`, ASAR `366c3166…a7bfba02`, [manifest](release-manifests/0.4.3-moe.30.json) with published=false. An independent local compile of the same revision reproduces the build's own compiled-output digests exactly for both trees, which is what covers the two repairs that leave no string literal. Package verification is now **PASS**: the downloaded installer and `app.asar` hashes both match the manifest, the packaged version is `0.4.3-moe.30`, every CLWX-130 and CLWX-102 literal marker is present in the shipped compiled bytes, and **172 of 172** compiled files are byte-identical to an independent local compile of the same revision — which is what covers the two repairs that leave no literal. **There is still no installed evidence: nothing is accepted.** moe.29 (`0f708082`) was built and artifact-verified but **never installed** — the system-initiated VM reboot destroyed the interactive QA session — and it contains none of the three repairs while shipping the browser code with both fail-opens live, so its results do not transfer. moe.28 remains the last candidate actually installed, with startup FAIL on the SQLite worker error and chat NOT_RUN. moe.25 retains the prior completed startup/Online baseline. ASR remains deferred.
...
## Current candidate — moe.30, built and provenance-verified; installed acceptance NOT_RUN

Source `40cfe727f38d1b29c4f5ff77e1eddba97556061f` on `release/moe30-integration`, worktree `/private/tmp/clawx-moe30-integration-20260909`, version `0.4.3-moe.30`, base `0f708082` (the moe.29 source). It exists because integration was the missed step, not the repairs: six commits across CLWX-61, CLWX-130 and CLWX-102 had each been independently approved and each verdict was valid for its own diff, but they lived only in author worktrees, so the frozen moe.29 installer contained none of them. For the browser lane moe.29 is a descendant of the rejected revision and its first correction while the fail-open fix is not an ancestor, so that artifact ships both reproduced fail-opens live. Every pick here is `git patch-id --stable` identical to the revision its own review approved.

Source gates at the candidate: typecheck clean; `lint:check` 0 errors (43 pre-existing warnings); full unit suite **2,444 passed / 29 skipped / 0 failed** across 216 files; comms replay + compare exit 0 with 45 PASS rows and 0.00% delta on `history_load_qps` and `rpc_p95_ms`; diff-aware validation of a purpose-built integration task spec; independent integration review **APPROVE**, re-confirmed against the moved tip with all 17 declared touched areas matching the 17 changed files.

[Build 34323058772](https://github.com/dmvevents/clawx-pilot/actions/runs/34323058772) succeeded with `head_sha` equal to the candidate SHA, keyless-public, publish job correctly skipped. Its own provenance receipt records a clean tree at `40cfe727` with no credential, cloud-gateway, Azure Speech or Graph seed included. [Manifest](release-manifests/0.4.3-moe.30.json): installer `9f8a2dc5fc5c238d49935a7f1b0b9b90205c104ddf933124196c62210114aff7` (476,773,051 bytes), `app.asar` `366c3166810434b083098f8517e3e8268325216851703394368dd9bea7bfba02` (260,765,109 bytes), plus hashed `bin` (8 files), `extensions` (29) and `openclaw-plugins` (4,933) trees; published=false.

**Reproducible-compile proof.** Recomputing the build's recorded compiled-output digests locally, with the repository's own algorithm, against an independent compile of the same revision returns identical hashes: `dist` `145ae584…5332825c` across 169 files and `dist-electron` `19b6c610…78fa5c34` across 3. This is the only evidence that covers CLWX-61's `if (!filename) continue;` and CLWX-130's path-containment replacement, because minification leaves them no distinguishing literal.

**Two green steps produced nothing.** The arm64 installer upload and the Windows update-manifest upload both report success while no such artifact exists — there was no arm64 build, and auto-update stays off in pilot mode. Only the x64 installer (473,569,882 bytes) can be verified or installed from this run; an arm64 claim would be unfounded.

Package verification **PASS**: installer SHA256 confirmed against the manifest by independent download, `app.asar` `366c3166…` confirmed, packaged `package.json` version `0.4.3-moe.30`, all CLWX-130 and CLWX-102 literal markers present in the shipped `dist-electron/main/index.js`, and **172/172** compiled files byte-identical to an independent compile of the same revision. One packaging observation was filed rather than waved through: 6 of 88 asar-header unpacked entries are absent from the payload (foreign-architecture `@napi-rs/canvas` prebuilds), which breaks whole-archive extraction — [CLWX-137](bugs/CLWX-137-asar-header-unpacked-mismatch.md), low severity, tooling-only, with the x64 runtime binding verified present. Installed upgrade, startup, ordinary chat, the doctor-repair path, tenant, client and unaided external-tester evidence are all **NOT_RUN** for this version, and no earlier version's evidence transfers to it. Installed acceptance is blocked by an operator dependency: it needs an interactive standard-user desktop on the QA instance, and that session was destroyed by a system-initiated reboot. Three Windows lanes were probed and closed.

## Current candidate — moe.29, artifact PASS; installed acceptance BLOCKED on environment

Source `0f708082c941fbed007173c57232aec916f4eff1` on `release/moe29-node-mode`, worktree `/private/tmp/clawx-clwx136-fix-20260909`, version `0.4.3-moe.29`. It integrates the independently reviewed CLWX-136 Gateway Node-mode entry shim (all three `utilityProcess.fork` sites, including the doctor auto-recovery) on top of the moe.28 lineage. Two Claude review lanes APPROVE — one verified the mechanism against a real Electron utilityProcess with a negative control that reproduces the defect class — and the Codex cross-model lane is clean after one spec-metadata fix. Full unit suite 2,332 passed / 51 skipped (one cold-start hook timeout recovered warm).

[Build 34303783279](https://github.com/dmvevents/clawx-pilot/actions/runs/34303783279) succeeded 02:49:47 UTC with `head_sha` equal to the candidate SHA, keyless-public. A first dispatch `34300195990` died at ~50 minutes on GitHub's runner-communication loss with no uploaded logs and is preserved as an infrastructure failure. Artifact gate PASS: **36/36** host package checks, 172 compiled-file comparisons, packaged OpenClaw 2026.9.2, both lifecycle markers absent, keyless seed scan empty, blockmap structure. Installer SHA256 `f48ac2f465a6a27a6dee385bb2c3b2e3d61d4e94ae4b633076427621fc5c4ce9` (476,772,726 bytes); ASAR `98407244b3202aad270af0ebdd8d7638bc9f82f7f886082d8e87eb5019c96a78`; packaged EXE `37d7fd1fe0e027421a32986c9241ee1b4364fab8267e31455f3d8556cc38b505`. The reviewed shim is present in the package at `resources/resources/gateway/` with SHA256 `4387e8e0a8b6c4bd360ad1e8c7f827e247e2f32e80c1eb29efa96a84cbf6750b`, byte-identical to source. Staged at `gs://clawx-rc-artifacts-622687731621/private-validation/moe29-34303783279/moe29.exe`; the QA VM downloaded it with exact hash and length at 02:59:38 UTC. published=false; no release manifest publication.

**Installed acceptance NOT_RUN and blocked on environment, not on the repair.** The VM rebooted at 03:29:42 UTC (system-initiated by `NT AUTHORITY\SYSTEM`), destroying the `ClawXFresh0908` interactive session. The prepared, parse-checked assisted-upgrade task returned `SCHED_S_TASK_HAS_NOT_RUN` and never executed; no partial installation exists. Profiles (`.openclaw` 642 files, app-data 61), the Chrome profile on disk, installed moe.28 identity, the staged installer and the RDP certificate pin `fe85ddd4…8c6e34` all survive. The 10 QA Chrome **processes** did not — earlier "preserved" wording is corrected. Re-establishing an interactive session requires the operator-held QA password. [CLWX-136 blocker and next action](bugs/CLWX-136-openclaw-sqlite-worker-startup.md).

## Current candidate — moe.28, upgrade PASS; SQLite worker startup FAIL

Source `c5e76b5dd69d43a632b8ac817b7f253ab76ec614` on `release/moe28-upgrade`, checkout `/private/tmp/clawx-plan-execution-20260908`. Reviewed installer/harness bytes passed all11 native standard-user scenarios and93 assertions without skips/timeouts. [Build34288617489](https://github.com/dmvevents/clawx-pilot/actions/runs/34288617489) succeeded23:17:20UTC:2,319 native tests passed/zero failed/57 skipped. All36 package checks and172 compiled-file comparisons pass; keyless/source/profile and packaged lifecycle-marker absence are verified. [Manifest](release-manifests/0.4.3-moe.28.json), published=false. Installer SHA256 `f74564761651c436141cd6bd0fdfdb25b561a950336f6ce736e400d0ce56994f` (476771752bytes); ASAR `730660d9816391b76b0bbb406862bef1764f3b716d8785500d5b042dc30dfa16`; EXE `6055c97a567e95358f430059c5b2443e03b509de59a737b1a94c676901adcbda`. Private GCS/native Windows download hash and length pass. Standard-user upgrade completed23:39:01UTC with native exit0 and exact EXE/ASAR hashes. All695 backup files matched. Both lifecycle markers are absent from the new runtime; the original marker remains hash-identical in the retained rollback copy. Run was verified unchecked before Finish. Actual shortcut launch23:39:28UTC failed the unchanged360-second observer:155 samples, native observer exit1, no stable ready state. OpenClaw reports `SQLite read-only worker returned invalid JSON`; ordinary chat is NOT_RUN. [CLWX-136](bugs/CLWX-136-openclaw-sqlite-worker-startup.md) owns diagnosis. Receipt: `artifacts/windows-vm/20260908-moe28/native-install-identity-lifecycle-result.json`.

## Previous installed candidate — moe.27, native install PASS; upgrade startup FAIL
...
Installer: `a5d080fa02ffe582652f05dc4459bbc439c131a48d44ca3c0f1395edd9ab38f3` (476769020 bytes). ASAR: `83e82f2850d13f97bbac40e6533600111e6023cec4d5ae1ede462420de0986eb`. Packaged EXE: `2749c2fe080a8370690b0968d9c00f355f57cce9129753f6b7238a8cc57c70b6`. Private GCS and native Windows download hashes/length pass. Standard-user installation completed at 20:49:10 UTC with native exit 0 and matching installed EXE/ASAR hashes. All 695 profile-backup files matched. The Run checkbox was verified unchecked before Finish; the actual desktop shortcut launched at 20:49:48 UTC. Startup failed the unchanged 360-second observation (159 samples, Gateway code 1, disabled composer). The installed runtime retains the exact old moe.26 lifecycle marker, absent from the new package; new launcher/package.json hashes match the payload. Root gracefully quit at 20:56:16 UTC, with no remaining app/listeners and the QA Chrome session retained. Installer replacement repair and independent/native testing are active under CLWX-135. This is assisted existing-profile Windows Server 2022 evidence; no GA release is claimed.
...
## Previous packaged and installed candidate — moe.26, startup FAIL
