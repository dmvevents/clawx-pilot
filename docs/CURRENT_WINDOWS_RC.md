...
**September 8 first-response RCA checkpoint, 07:22Z:** moe.24 (`b814f804`, run `34189597051`) has verified installed identity but fails ordinary Online response acceptance. CLWX-125 owns the defect. Exact-session evidence shows external cancellation after 112 seconds of preparation. A same-build diagnostic measures 54.133 seconds in SDK alias materialization, but reused an old local-model session despite displaying Online; it is not Online acceptance. Reviewed history repair `f5875b54` and UI repair `aa398dfb` pass source checks. SDK repair `ba459ef1` also passes a native Windows fixture: 296 unchanged writes become zero with byte-identical wrappers. An SDK-only installed diagnostic is active with original runtime files backed up; session-freshness verification is being repaired before further acceptance. No repaired artifact handoff; **GA remains RED**. The [RCA and flow map](evidence/WINDOWS_FIRST_RESPONSE_RCA_2026-09-08.md) records known-good commits, code samples, tests, VM setup and limitations.
