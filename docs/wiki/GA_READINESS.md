...
| G7 | 2026-07-20 | Ansari: "only thing I absolutely need is the redirect URIs" — **still unsent, ~6wk** (LIAISON_LOG §B) | Value drafted in reply doc | 🔴 OPEN — owner-gated send (KR8) |
| G8 | 2026-08-18 | Ministry handoff: 4 design conflicts (app-server vs desktop, secret vs PKCE, redirect URI, read-only scopes); all 20 values placeholders (`MINISTRY_INFRA_HANDOFF_2026-08-18.md`) | 459-line reply resolves all 4 — UNSENT | 🔴 OPEN — the Lane-C unlock (KR8→KR7→KR6) |
...
| G15 | 2026-09-01 | Outbound-message ledger gap: we can't always prove what we told Raj (LIAISON_LOG §F) | Treat `outbound-drafts/` as intent; log every send going forward | 🟡 ACCEPTED RISK for GA (process fix, not code) |
