...
| COMPOSER-OVERRIDE | **REPRODUCED LIVE 2026-09-03 04:15** (was "UNVERIFIED — likely stale"): owner screenshot shows the composer pinned to `gemini-2.5-flash` on Online → "Model call failed / 400 status code (no body)", gateway connected. Demo-blocking. `clawx-config-doctor` dispatched for the four-store repair; recorded failure-state evidence (video + screenshot): `skills/laptop/evidence/2026-09-03-app-usecase-recorded/`. |
