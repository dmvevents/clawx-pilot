...
When the principal asks to open Chrome or the browser, the assistant calls the explicit `browser.open_chrome` tool, which opens the principal's system Chrome through the existing Main repair service (`POST /api/browser/repair-chrome-cdp`); the stock managed browser is never used for Ministry journeys. On Windows, a responding CDP endpoint is reported ready only after a bounded loopback ownership check confirms the listener is a Chrome ClawX may drive: the Chrome ClawX itself spawned (positive PID ownership), the dedicated ClawX automation profile, or the principal's own user-profile Chrome deliberately started with the debug port. The Outlook and Forms drivers run the same ownership check before attaching over CDP, and ownership, launch and readiness derive one port identity from the endpoint. A confirmed different-session owner and a same-session conflict return distinct truthful guidance (`foreign_endpoint_owner`), and an unidentifiable owner is refused as `endpoint_owner_unverified` instead of a false ready — the principal is never told to sign in to a Windows session they are already in, unknown identity never causes a kill, and ClawX never closes or attaches to another user's Chrome or switches ports silently.
