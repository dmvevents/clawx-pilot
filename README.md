...
When the principal asks to open Chrome or the browser, the assistant calls the explicit `browser.open_chrome` tool, which opens the principal's system Chrome through the existing Main repair service (`POST /api/browser/repair-chrome-cdp`); the stock managed browser is never used for Ministry journeys. On Windows, a responding CDP endpoint is reported ready only after a bounded loopback ownership check confirms the listener runs in the current Windows session on the dedicated ClawX automation profile. A confirmed foreign session/profile or an unidentifiable owner returns a typed refusal (`foreign_endpoint_owner` / `endpoint_owner_unverified`) with Windows-appropriate guidance instead of a false ready — ClawX never closes or attaches to another user's Chrome and never switches ports silently.
